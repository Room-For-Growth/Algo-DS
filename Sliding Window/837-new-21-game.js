...
入选理由
dp + 滑动窗口的结合。让大家理解滑动窗口是如何和其他技巧联系的。 之前我也出了 二分 + dfs 这种联动性题目，这种题目对于你学习知识很有帮助。
...
而我们要求的就是分数大于等于 K 的这些情况中不大于 N 的概率，即满足 K<= x <=N 中的概率，其中 x 为分数。也就是求满足 K<= x <=N 的总个数再除以 W，因为一次抽取有 W 种可能，分别是 [1,W]，且概率均相等，都为 1 /W。
...
时间复杂度是 O(KW)O(KW)，代入题目的限制条件 0 <= K <= N <= 10000，1 <= W <= 10000，得到总的计算次数大约是 10^8，大于 10^7，因此会超时。

通过观察发现，每次 sum 我们变化的其实仅仅是左右两侧的数，中间的是不会变的。这不就是我们讲义中的滑动窗口使用场景么？

也就是说，我们只需要预先计算窗口的总和，之后更新窗口的时候都减去窗口右侧过期的数，再加上窗口左侧刚进去的数就行了。这种算法的时间复杂度是 O(W + K)O(W+K)
*/
